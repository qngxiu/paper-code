...
rm(list = ls())
getwd()
setwd("C:/Users/19721/Desktop/重金属实验/修正后数据/比对")
xiufu <- read.csv("比对后ba.csv",header = T,row.names = 1)
qian <- read.csv("o_ba.csv",row.names = 1)
df1 <- xiufu
merged_df <- df1
print(merged_df)
set.seed(123)
...
p <- ggplot(pcoa_result, aes(x = PCoA1, y = PCoA2, color = Group)) +
  ...
  scale_colour_manual(values = c("#c0c9e7", "#eedb8c", "#8a69a8","grey", "#cdd6b6")) +
  scale_fill_manual(values = c("#c0c9e7", "#eedb8c", "#8a69a8","grey", "#cdd6b6")) +
  theme(
...
getwd()
setwd("C:/Users/19721/Desktop/重金属实验/修正后数据/比对")
xiufu <- read.csv("比对后fu.csv",header = T,row.names = 1)
qian <- read.csv("o_ba.csv",row.names = 1)
df1 <- xiufu
merged_df <- df1
print(merged_df)
